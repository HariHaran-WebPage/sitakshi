...
/* ══════════════════════════════════════════════
   HERO RIGHT — Fixed layout, no overlaps
   Layout zones (total height: 560px, width: 600px):
   - Row 1 (top, y 0–90):   4 stat mini-cards side by side
   - Row 2 (mid, y 90–400): social icons on sides + sphere center
   - Row 3 (bot, y 400–480): live analytics + weekly growth cards side by side
   - Row 4 (bot, y 480–560): badge strip
══════════════════════════════════════════════ */
function HeroRight() {
  ...
  // Social icons: 3 on each side of the sphere, vertically spread in the middle zone
  const leftIcons = [
    {
      label: 'Facebook', bg: '#1877F2',
      size: 48, delay: '0s', dur: '3.4s', dir: 1,
      svg: <svg width="22" height="22" viewBox="0 0 24 24" fill="#fff"><path d="M18 2h-3a5 5 0 0 0-5 5v3H7v4h3v8h4v-8h3l1-4h-4V7a1 1 0 0 1 1-1h3z"/></svg>,
    },
    {
      label: 'WhatsApp', bg: '#25D366',
      size: 44, delay: '0.3s', dur: '4.5s', dir: -1,
      svg: <svg width="20" height="20" viewBox="0 0 24 24" fill="#fff"><path d="M21 11.5a8.38 8.38 0 0 1-.9 3.8 8.5 8.5 0 0 1-7.6 4.7 8.38 8.38 0 0 1-3.8-.9L3 21l1.9-5.7a8.38 8.38 0 0 1-.9-3.8 8.5 8.5 0 0 1 4.7-7.6 8.38 8.38 0 0 1 3.8-.9h.5a8.48 8.48 0 0 1 8 8v.5z"/></svg>,
    },
    {
      label: 'Telegram', bg: '#2AABEE',
      size: 42, delay: '1.4s', dur: '4.1s', dir: 1,
      svg: <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#fff" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><line x1="22" y1="2" x2="11" y2="13"/><polygon points="22 2 15 22 11 13 2 9 22 2"/></svg>,
    },
  ];
  ...
  const rightIcons = [
    {
      ...
      size: 50, delay: '0.5s', dur: '3.9s', dir: 1,
      svg: <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#fff" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="2" y="2" width="20" height="20" rx="5" ry="5"/><path d="M16 11.37A4 4 0 1 1 12.63 8 4 4 0 0 1 16 11.37z"/><line x1="17.5" y1="6.5" x2="17.51" y2="6.5"/></svg>,
    },
    {
      label: 'Twitter/X', bg: '#000',
      size: 44, delay: '0.7s', dur: '3.8s', dir: -1,
      svg: <svg width="20" height="20" viewBox="0 0 24 24" fill="#fff"><path d="M17.5 4h3L13 10.5 20 20h-3L12 13.5 7 20H4l7-7L4 4h3l5.5 7L17.5 4z" fill="#fff"/></svg>,
    },
    {
      label: 'Pinterest', bg: '#E60023',
      size: 42, delay: '1.0s', dur: '4.3s', dir: 1,
      svg: <svg width="20" height="20" viewBox="0 0 24 24" fill="#fff"><path d="M12 2C6.48 2 2 6.48 2 12c0 4.24 2.65 7.86 6.39 9.29-.09-.78-.17-1.98.04-2.83.18-.77 1.22-5.17 1.22-5.17s-.31-.62-.31-1.54c0-1.44.84-2.52 1.88-2.52.89 0 1.32.67 1.32 1.47 0 .9-.57 2.24-.87 3.49-.25 1.04.52 1.89 1.55 1.89 1.86 0 3.29-1.96 3.29-4.79 0-2.51-1.8-4.26-4.37-4.26-2.98 0-4.73 2.23-4.73 4.54 0 .9.35 1.86.78 2.39.09.1.1.19.07.29-.08.33-.26 1.04-.29 1.18-.05.19-.16.23-.38.14-1.39-.65-2.26-2.68-2.26-4.31 0-3.51 2.55-6.74 7.35-6.74 3.86 0 6.86 2.75 6.86 6.42 0 3.83-2.41 6.91-5.76 6.91-1.13 0-2.19-.59-2.55-1.28l-.69 2.6c-.25.97-.93 2.18-1.39 2.92A10 10 0 0 0 12 22c5.52 0 10-4.48 10-10S17.52 2 12 2z"/></svg>,
    },
  ];

  // Top social icons (LinkedIn + YouTube) centered above sphere
  const topIcons = [
    {
      label: 'LinkedIn', bg: '#0A66C2',
      size: 46, delay: '0.8s', dur: '4.2s', dir: -1,
      svg: <svg width="22" height="22" viewBox="0 0 24 24" fill="#fff"><path d="M16 8a6 6 0 0 1 6 6v7h-4v-7a2 2 0 0 0-2-2 2 2 0 0 0-2 2v7h-4v-7a6 6 0 0 1 6-6z"/><rect x="2" y="9" width="4" height="12"/><circle cx="4" cy="4" r="2"/></svg>,
    },
    {
      label: 'YouTube', bg: '#FF0000',
      size: 46, delay: '1.1s', dur: '3.6s', dir: 1,
      svg: <svg width="22" height="22" viewBox="0 0 24 24" fill="#fff"><path d="M22.54 6.42a2.78 2.78 0 0 0-1.95-1.96C18.88 4 12 4 12 4s-6.88 0-8.59.46A2.78 2.78 0 0 0 1.46 6.42 29 29 0 0 0 1 12a29 29 0 0 0 .46 5.58A2.78 2.78 0 0 0 3.41 19.6C5.12 20 12 20 12 20s6.88 0 8.59-.46a2.78 2.78 0 0 0 1.95-1.95A29 29 0 0 0 23 12a29 29 0 0 0-.46-5.58z"/><polygon points="9.75 15.02 15.5 12 9.75 8.98 9.75 15.02" fill="#FF0000"/></svg>,
    },
  ];
  ...
  return (
    <div style={{ position: 'relative', width: 600, height: 560 }}>
      ...
      <svg style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', pointerEvents: 'none', zIndex: 0 }} viewBox="0 0 600 560" xmlns="http://www.w3.org/2000/svg">
        {/* Lines from side icon zones toward sphere center ~300,240 */}
        {/* Left column icons at x≈52 */}
      ...
      {/* ══ ROW 1: Top stat cards (4 across, above everything) ══ */}
      <div style={{ position: 'absolute', top: 0, left: 0, right: 0, display: 'flex', gap: 8, zIndex: 8, animation: 'slideInLeftHR 0.7s ease 0.2s both' }}>
        {/* Followers */}
        <div style={{ flex: 1, background: 'rgba(255,255,255,0.92)', backdropFilter: 'blur(14px)', border: '1px solid rgba(0,163,77,0.22)', borderRadius: 12, padding: '10px 12px', boxShadow: '0 4px 16px rgba(0,0,0,0.08)' }}>
          <div style={{ fontSize: 9, color: T.textLighter, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.4px' }}>Followers</div>
          <div style={{ fontSize: 20, fontWeight: 800, color: T.primary, lineHeight: 1.1, marginTop: 2 }}>+12K</div>
          <div style={{ fontSize: 9, color: T.primary, fontWeight: 600, marginTop: 2 }}>▲ this month</div>
        ...
        {/* Engagement */}
        <div style={{ flex: 1, background: 'rgba(255,255,255,0.92)', backdropFilter: 'blur(14px)', border: '1px solid rgba(0,163,77,0.22)', borderRadius: 12, padding: '10px 12px', boxShadow: '0 4px 16px rgba(0,0,0,0.08)' }}>
          <div style={{ fontSize: 9, color: T.textLighter, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.4px' }}>Engagement</div>
          <div style={{ fontSize: 20, fontWeight: 800, color: T.primaryDark, lineHeight: 1.1, marginTop: 2 }}>+85%</div>
          <div style={{ fontSize: 9, color: T.primary, fontWeight: 600, marginTop: 2 }}>▲ rate increase</div>
        </div>
        {/* Leads */}
        <div style={{ flex: 1, background: 'rgba(255,255,255,0.92)', backdropFilter: 'blur(14px)', border: '1px solid rgba(0,163,77,0.22)', borderRadius: 12, padding: '10px 12px', boxShadow: '0 4px 16px rgba(0,0,0,0.08)' }}>
          <div style={{ fontSize: 9, color: T.textLighter, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.4px' }}>Leads</div>
          <div style={{ fontSize: 20, fontWeight: 800, color: '#005c24', lineHeight: 1.1, marginTop: 2 }}>+350</div>
          <div style={{ fontSize: 9, color: T.primary, fontWeight: 600, marginTop: 2 }}>▲ new leads</div>
        </div>
        {/* Conversion */}
        <div style={{ flex: 1, background: 'rgba(255,255,255,0.92)', backdropFilter: 'blur(14px)', border: '1px solid rgba(0,163,77,0.22)', borderRadius: 12, padding: '10px 12px', boxShadow: '0 4px 16px rgba(0,0,0,0.08)' }}>
          <div style={{ fontSize: 9, color: T.textLighter, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.4px' }}>Conversion</div>
          <div style={{ fontSize: 20, fontWeight: 800, color: T.primary, lineHeight: 1.1, marginTop: 2 }}>+42%</div>
          <div style={{ fontSize: 9, color: T.primary, fontWeight: 600, marginTop: 2 }}>▲ rate boost</div>
        </div>
      </div>

      {/* ══ ROW 2: Middle zone — left icons | sphere | right icons ══ */}
      {/* Top center icons (LinkedIn + YouTube) */}
      <div style={{ position: 'absolute', top: 90, left: '50%', transform: 'translateX(-50%)', display: 'flex', gap: 14, zIndex: 5, alignItems: 'center' }}>
        {topIcons.map(ic => (
          <div key={ic.label} style={{ width: ic.size, height: ic.size, borderRadius: '50%', background: ic.bg, display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 4px 16px rgba(0,0,0,0.18)', animation: `${ic.dir > 0 ? 'floatUpHR' : 'floatDnHR'} ${ic.dur} ease-in-out infinite ${ic.delay}` }} title={ic.label}>{ic.svg}</div>
        ))}
      </div>

      {/* Left column icons — evenly spaced in the middle zone */}
      {leftIcons.map((ic, i) => (
        <div key={ic.label} style={{ position: 'absolute', top: 130 + i * 92, left: 8, width: ic.size, height: ic.size, borderRadius: '50%', background: ic.bg, display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 4px 20px rgba(0,0,0,0.18)', zIndex: 5, animation: `${ic.dir > 0 ? 'floatUpHR' : 'floatDnHR'} ${ic.dur} ease-in-out infinite ${ic.delay}` }} title={ic.label}>{ic.svg}</div>
      ))}

      {/* Right column icons */}
      {rightIcons.map((ic, i) => (
        <div key={ic.label} style={{ position: 'absolute', top: 130 + i * 92, right: 8, width: ic.size, height: ic.size, borderRadius: '50%', background: ic.bg, display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 4px 20px rgba(0,0,0,0.18)', zIndex: 5, animation: `${ic.dir > 0 ? 'floatUpHR' : 'floatDnHR'} ${ic.dur} ease-in-out infinite ${ic.delay}` }} title={ic.label}>{ic.svg}</div>
      ))}

      {/* ── Outer glow behind sphere ── */}
      <div style={{ position: 'absolute', left: '50%', top: 255, transform: 'translate(-50%,-50%)', width: 320, height: 320, borderRadius: '50%', background: 'radial-gradient(ellipse, rgba(0,163,77,0.18) 0%, rgba(0,163,77,0.06) 52%, transparent 75%)', animation: 'glowPulseHR 2.8s ease-in-out infinite', zIndex: 1, pointerEvents: 'none' }}/>

      {/* ── Orbital rings ── */}
      <div style={{ position: 'absolute', left: '50%', top: 255, transform: 'translate(-50%,-50%)', width: 300, height: 300, zIndex: 2, pointerEvents: 'none' }}>
        <div style={{ position: 'absolute', inset: 0, borderRadius: '50%', border: '1.5px solid rgba(0,163,77,0.38)', transform: 'rotateX(70deg)', animation: 'spinRingHR 9s linear infinite', transformStyle: 'preserve-3d' }}/>
        <div style={{ position: 'absolute', inset: '8px', borderRadius: '50%', border: '1.5px dashed rgba(0,180,80,0.28)', transform: 'rotateX(50deg) rotateY(20deg)', animation: 'spinRingHR2 13s linear infinite reverse', transformStyle: 'preserve-3d' }}/>
        <div style={{ position: 'absolute', inset: '16px', borderRadius: '50%', border: '1px solid rgba(0,200,100,0.18)', transform: 'rotateX(82deg) rotateZ(45deg)', animation: 'spinRingHR 18s linear infinite reverse', transformStyle: 'preserve-3d' }}/>
      </div>

      {/* ── THE SPHERE ── */}
      <div style={{ position: 'absolute', left: '50%', top: 255, transform: 'translate(-50%,-50%)', width: 230, height: 230, borderRadius: '50%', background: 'radial-gradient(ellipse at 34% 27%, #ffffff 0%, #e4f8ef 26%, #aadfc5 56%, #5db88a 78%, #268c52 100%)', boxShadow: '10px 16px 48px rgba(0,100,40,0.34), inset -14px -10px 30px rgba(0,80,30,0.16), inset 10px 8px 24px rgba(255,255,255,0.55)', zIndex: 3, overflow: 'visible' }}>
        ...
        <div style={{ position: 'absolute', bottom: -18, left: '50%', transform: 'translateX(-50%)', width: '72%', height: 22, borderRadius: '50%', background: 'rgba(0,100,40,0.18)', filter: 'blur(8px)', zIndex: -1 }}/>
      </div>

      {/* ── LIKE notification — floats just below sphere center, left of center ── */}
      ...
      {/* ── DM notification — right of center, same level ── */}
      <div style={{ position: 'absolute', top: 310, left: '50%', marginLeft: 30, background: 'rgba(255,255,255,0.92)', backdropFilter: 'blur(10px)', border: '1px solid rgba(0,163,77,0.2)', borderRadius: 14, padding: '8px 12px', zIndex: 7, boxShadow: '0 4px 18px rgba(0,0,0,0.10)', maxWidth: 158, animation: 'slideInRightHR 0.7s ease 1.1s both' }}>
      ...
      {/* ══ ROW 3: Analytics cards side by side below sphere ══ */}
      <div style={{ position: 'absolute', top: 388, left: 0, right: 0, display: 'flex', gap: 10, zIndex: 8 }}>
        {/* Live Analytics card */}
        <div style={{ flex: 1, background: 'rgba(10,20,14,0.94)', backdropFilter: 'blur(14px)', border: '1px solid rgba(0,163,77,0.32)', borderRadius: 14, padding: '11px 13px', boxShadow: '0 8px 32px rgba(0,0,0,0.3)', animation: 'slideInLeftHR 0.7s ease 0.6s both' }}>
        ...
        {/* Weekly Growth card */}
        <div style={{ flex: 1, background: 'rgba(255,255,255,0.92)', backdropFilter: 'blur(12px)', border: '1px solid rgba(0,163,77,0.22)', borderRadius: 14, padding: '11px 13px', boxShadow: '0 5px 20px rgba(0,0,0,0.10)', animation: 'slideInRightHR 0.7s ease 0.8s both' }}>
        ...
        {/* Campaigns badge card */}
        <div style={{ flex: 1, background: 'rgba(255,255,255,0.92)', backdropFilter: 'blur(12px)', border: '1px solid rgba(0,163,77,0.22)', borderRadius: 14, padding: '11px 13px', boxShadow: '0 5px 20px rgba(0,0,0,0.10)', animation: 'slideInRightHR 0.7s ease 1.0s both' }}>
      ...
      {/* ══ ROW 4: Bottom badge strip ══ */}
      <div style={{ position: 'absolute', bottom: 0, left: '50%', transform: 'translateX(-50%)', display: 'flex', gap: 8, zIndex: 8, animation: 'slideInLeftHR 0.7s ease 1s both', whiteSpace: 'nowrap' }}>
        {[
          { val: '580%', label: 'Avg. Engagement Lift' },
          { val: '30d',  label: 'To First Viral Post' },
          { val: '2.4M+', label: 'Followers Grown' },
        ].map(b => (
    ...
    <div style={{ display: 'flex', justifyContent: 'center' }}>
      <div ref={ref} style={{ width: 260, transform: `perspective(900px) rotateX(${-tilt.y * 0.5}deg) rotateY(${tilt.x * 0.5}deg)`, transition: 'transform 0.2s ease-out' }}>
        <div style={{ background: 'linear-gradient(165deg,#0a1610,#06100a)', border: '1px solid rgba(0,163,77,0.24)', borderRadius: 16, padding: 16, boxShadow: '0 28px 56px -16px rgba(0,0,0,0.55)', position: 'relative', overflow: 'hidden' }}>
          ...
          {[{ kw: 'Reels avg. watch time', pos: '4.8s', delta: '↑22%' }, { kw: 'Engagement rate', pos: '5.2%', delta: '↑1.4%' }, { kw: 'Follower growth/wk', pos: '+1.2K', delta: '↑38%' }].map((row, i) => (
    ...
    <div style={{ position: 'relative', overflow: 'hidden', background: T.bgLight }}>
      ...
      <div className="sm-hero-grid" style={{ maxWidth: 1320, margin: '0 auto', padding: '80px 64px 72px', display: 'grid', gridTemplateColumns: '1fr 1.1fr', gap: 48, alignItems: 'center', position: 'relative', zIndex: 2 }}>
        ...
        <div>
          ...
          <h1 className="sm-reveal" style={{ fontFamily: 'Playfair Display,serif', fontSize: 56, fontWeight: 900, lineHeight: 1.06, color: T.text, letterSpacing: '-1.2px', marginBottom: 20, animationDelay: '0.1s' }}>
...
function StatsBand() {
  ...
  return (
    <div style={{ background: T.text, position: 'relative', overflow: 'hidden' }}>
      <div style={{ position: 'absolute', top: 0, left: 0, right: 0, height: 2, background: `linear-gradient(90deg,transparent,${T.primaryDark},${T.primary},#33c972,${T.primary},${T.primaryDark},transparent)`, backgroundSize: '200% 100%', animation: 'gradientMove 3s linear infinite' }}/>
...
function ServicesGrid() {
  return (
    <div id="services" style={{ maxWidth: 1320, margin: '96px auto', padding: '0 64px' }}>
      ...
      <h2 style={{ fontFamily: 'Playfair Display,serif', fontSize: 38, fontWeight: 900, color: T.text, marginBottom: 8, lineHeight: 1.1, letterSpacing: '-1px' }}>Social That Actually Converts</h2>
  ...
  return (
    <div id="work" style={{ background: T.bgLight, padding: '88px 64px', borderTop: `1px solid ${T.border}`, borderBottom: `1px solid ${T.border}` }}>
      <div style={{ maxWidth: 1320, margin: '0 auto' }}>
        ...
        <h2 style={{ fontFamily: 'Playfair Display,serif', fontSize: 38, fontWeight: 900, color: T.text, marginBottom: 8, lineHeight: 1.1, letterSpacing: '-1px' }}>Results Speak Louder</h2>
...
function Contact() {
  ...
  return (
    <div id="contact" style={{ maxWidth: 1320, margin: '96px auto', padding: '0 64px' }}>
      ...
      <h2 style={{ fontFamily: 'Playfair Display,serif', fontSize: 38, fontWeight: 900, color: T.text, marginBottom: 8, lineHeight: 1.1, letterSpacing: '-1px' }}>Get Your Free Social Audit</h2>
    ...
    <>
      <style>{`
        ...
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0;}
        ...
        @keyframes glowPulseHR{0%,100%{opacity:0.55}50%{opacity:0.88}}
        @keyframes spinRingHR{from{transform:rotateX(70deg) rotateZ(0deg)}to{transform:rotateX(70deg) rotateZ(360deg)}}
        @keyframes spinRingHR2{from{transform:rotateX(50deg) rotateY(20deg) rotateZ(0deg)}to{transform:rotateX(50deg) rotateY(20deg) rotateZ(-360deg)}}
        @keyframes floatUpHR{0%,100%{transform:translateY(0)}50%{transform:translateY(-10px)}}
        @keyframes floatDnHR{0%,100%{transform:translateY(0)}50%{transform:translateY(9px)}}
        @keyframes slideInLeftHR{from{opacity:0;transform:translateX(-18px)}to{opacity:1;transform:translateX(0)}}
        @keyframes slideInRightHR{from{opacity:0;transform:translateX(18px)}to{opacity:1;transform:translateX(0)}}
        @keyframes pulseDot{0%,100%{box-shadow:0 0 0 3px rgba(0,163,77,0.25)}50%{box-shadow:0 0 0 6px rgba(0,163,77,0.1)}}
        ...
        .enq-input:focus{border-color:${T.primary};background:rgba(0,163,77,0.08);}
        ...
        .enq-select-wrap:focus-within .enq-select-chevron{color:${T.primary};transform:rotate(180deg);}
        .enq-select option{background:#071209;color:#fff;}

        @media(max-width:1024px){
          .sm-hero-grid{grid-template-columns:1fr !important;padding:56px 40px 48px !important;gap:40px !important;}
        ...
        @media(max-width:640px){
          .sm-hero-grid{padding:40px 20px 36px !important;gap:28px !important;}
