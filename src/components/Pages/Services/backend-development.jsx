  ...
  return (
    <>
      <nav style={{
        ...
        <div style={{ maxWidth: 1320, margin: '0 auto', padding: '0 48px', height: 64, display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 24 }} className="nav-inner">
          {/* Logo */}
          <a href="#" style={{ display: 'flex', alignItems: 'center', gap: 10, textDecoration: 'none', flexShrink: 0 }}>
            <div style={{ width: 34, height: 34, background: '#0a0a0a', borderRadius: 9, display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#22c55e', fontSize: 17 }}>
              <i className="ti ti-server-2" />
            </div>
            <span style={{ fontFamily: "'Playfair Display',serif", fontSize: 17, fontWeight: 900, color: '#0a0a0a', letterSpacing: '-0.3px' }}>
              Back<span style={{ color: '#22c55e' }}>end</span>Co
            </span>
          </a>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 4 }} className="nav-links-desktop">
            {navLinks.map(l => (
              <button key={l.id} onClick={() => scrollTo(l.id)} style={{ fontSize: 13, fontWeight: 500, color: '#374151', background: 'none', border: 'none', cursor: 'pointer', padding: '7px 13px', borderRadius: 8, fontFamily: "'Poppins',sans-serif", transition: 'background 0.2s,color 0.2s', whiteSpace: 'nowrap' }}
          ...
          {/* Desktop Actions */}
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexShrink: 0 }} className="nav-actions-desktop">
            <button style={{ fontFamily: "'Poppins',sans-serif", fontSize: 12.5, fontWeight: 600, color: '#374151', background: 'transparent', border: '1.5px solid #e2e8f0', borderRadius: 9, padding: '8px 16px', cursor: 'pointer', transition: 'all 0.2s', whiteSpace: 'nowrap' }}
              onMouseEnter={e => { e.currentTarget.style.borderColor = '#16a34a'; e.currentTarget.style.color = '#16a34a'; }}
              onMouseLeave={e => { e.currentTarget.style.borderColor = '#e2e8f0'; e.currentTarget.style.color = '#374151'; }}>
              View Our Work
            </button>
            <button onClick={() => scrollTo('contact')} style={{ fontFamily: "'Poppins',sans-serif", fontSize: 12.5, fontWeight: 600, color: '#fff', background: '#0a0a0a', border: 'none', borderRadius: 9, padding: '8px 18px', cursor: 'pointer', transition: 'all 0.2s', whiteSpace: 'nowrap' }}
              onMouseEnter={e => { e.currentTarget.style.background = '#1a2e1a'; e.currentTarget.style.transform = 'translateY(-1px)'; }}
          ...
          {/* Hamburger */}
          <button onClick={() => setMenuOpen(o => !o)} className="nav-hamburger-btn" style={{ display: 'none', background: 'none', border: '1.5px solid #e2e8f0', borderRadius: 8, padding: '6px 8px', cursor: 'pointer', fontSize: 18, color: '#374151', lineHeight: 1, alignItems: 'center', justifyContent: 'center' }}>
      ...
      {/* Mobile Menu */}
      {menuOpen && (
        <div style={{ position: 'fixed', top: 64, left: 0, right: 0, background: 'rgba(250,250,248,0.97)', backdropFilter: 'blur(16px)', WebkitBackdropFilter: 'blur(16px)', borderBottom: '1px solid #e2e8f0', padding: '16px 24px 24px', display: 'flex', flexDirection: 'column', gap: 4, zIndex: 99 }}>
          {navLinks.map(l => (
            <button key={l.id} onClick={() => scrollTo(l.id)} style={{ fontSize: 14, fontWeight: 500, color: '#374151', background: 'none', border: 'none', cursor: 'pointer', padding: '11px 14px', borderRadius: 9, fontFamily: "'Poppins',sans-serif", textAlign: 'left', transition: 'background 0.2s,color 0.2s' }}
              onMouseEnter={e => { e.currentTarget.style.background = '#f0fdf4'; e.currentTarget.style.color = '#15803d'; }}
              onMouseLeave={e => { e.currentTarget.style.background = 'none'; e.currentTarget.style.color = '#374151'; }}>
              {l.label}
            </button>
          ...
          <div style={{ display: 'flex', gap: 10 }}>
            <button style={{ flex: 1, fontFamily: "'Poppins',sans-serif", fontSize: 12.5, fontWeight: 600, color: '#374151', background: 'transparent', border: '1.5px solid #e2e8f0', borderRadius: 9, padding: '10px 16px', cursor: 'pointer' }}>View Our Work</button>
            <button onClick={() => scrollTo('contact')} style={{ flex: 1, fontFamily: "'Poppins',sans-serif", fontSize: 12.5, fontWeight: 600, color: '#fff', background: '#0a0a0a', border: 'none', borderRadius: 9, padding: '10px 16px', cursor: 'pointer' }}>Get In Touch</button>
          </div>
        </div>
      )}
    </>
...
function MacBookMockup() {
  return (
    <div style={{ width: '100%', maxWidth: 580, margin: '0 auto', position: 'relative', transform: 'perspective(1600px) rotateY(-8deg) rotateX(4deg)', transformOrigin: 'center center' }}>
        ...
        <div style={{ margin: '7px 7px 0', background: '#0a0b0f', borderRadius: '9px 9px 0 0', padding: '10px 10px 0', position: 'relative', boxShadow: 'inset 0 0 0 1px rgba(0,0,0,0.8),inset 0 2px 4px rgba(0,0,0,0.6)' }}>
          <div style={{ textAlign: 'center', marginBottom: 6 }}>
            <div style={{ display: 'inline-flex', alignItems: 'center', gap: 5 }}>
              {['#1a1d22', '#252830', '#1a1d22'].map((bg, i) => (
                <div key={i} style={{ width: i === 1 ? 5 : 4, height: i === 1 ? 5 : 4, borderRadius: '50%', background: bg, border: '0.5px solid rgba(255,255,255,0.06)' }} />
              ))}
            </div>
          </div>
      ...
      <div style={{ position: 'relative', height: 5, background: 'linear-gradient(180deg,#1a1d23 0%,#252830 40%,#1e2128 100%)' }}>
        <div style={{ position: 'absolute', top: 0, left: '5%', right: '5%', height: 1, background: 'linear-gradient(90deg,transparent,rgba(0,0,0,0.8),rgba(0,0,0,0.8),transparent)' }} />
      </div>
      <div style={{ background: 'linear-gradient(180deg,#2c2f37 0%,#252830 30%,#1e2028 70%,#191b22 100%)', borderRadius: '0 0 12px 12px', height: 28, position: 'relative', boxShadow: 'inset 0 1px 0 rgba(255,255,255,0.1),inset 0 -1px 0 rgba(0,0,0,0.5)', overflow: 'hidden' }}>
        <div style={{ position: 'absolute', top: 5, left: '8%', right: '8%', display: 'flex', flexDirection: 'column', gap: 2.5 }}>
          {[{ w: '100%', keys: 14 }, { w: '96%', keys: 12 }, { w: '92%', keys: 11 }].map((row, i) => (
            <div key={i} style={{ width: row.w, margin: '0 auto', height: 3, borderRadius: 1.5, background: 'rgba(255,255,255,0.045)', boxShadow: 'inset 0 1px 0 rgba(255,255,255,0.05),inset 0 -1px 0 rgba(0,0,0,0.3)' }}>
              <div style={{ display: 'flex', gap: 2, padding: '0 4px', height: '100%', alignItems: 'center' }}>
                {Array.from({ length: row.keys }).map((_, j) => (
                  <div key={j} style={{ flex: 1, height: '80%', borderRadius: 1, background: 'rgba(255,255,255,0.06)' }} />
                ))}
              </div>
            </div>
          ))}
        </div>
        <div style={{ position: 'absolute', bottom: 3, left: '50%', transform: 'translateX(-50%)', width: 52, height: 11, borderRadius: 3, background: 'rgba(255,255,255,0.035)', border: '0.5px solid rgba(255,255,255,0.05)' }} />
      </div>
      <div style={{ height: 5, background: 'linear-gradient(180deg,#1a1c22 0%,#131519 100%)', borderRadius: '0 0 14px 14px', boxShadow: '0 4px 16px rgba(0,0,0,0.45)' }} />
    </div>
  );
}
...
function MockThumb({ type, color }) {
  const base = { width: '100%', height: 92, borderRadius: 12, background: 'rgba(255,255,255,0.035)', border: '1px solid rgba(255,255,255,0.07)', position: 'relative', overflow: 'hidden', marginBottom: 18 };
  const baseLight = { ...base, background: 'rgba(0,0,0,0.025)', border: '1px solid rgba(0,0,0,0.06)' };
  const isLight = color === '#15803d' || color === '#16a34a';
  const wrap = isLight ? baseLight : base;

  if (type === 'api') return (
    <div style={{ ...wrap, padding: '10px 12px', fontFamily: "'JetBrains Mono',monospace" }}>
  ...
  if (type === 'db') return (
    <div style={wrap}>
      <svg viewBox="0 0 220 92" width="100%" height="100%">
        {[18, 40, 62].map((y, idx) => <ellipse key={idx} cx="110" cy={y} rx="46" ry="8" fill="none" stroke={color} strokeWidth="1.4" opacity={0.7 - idx * 0.15} />)}
  ...
  if (type === 'queue') return (
    <div style={{ ...wrap, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 4 }}>
      {[0, 1, 2, 3, 4].map(idx => (
        <div key={idx} style={{ width: 22, height: 36, borderRadius: 4, background: color, opacity: idx === 4 ? 0.9 : 0.25 + idx * 0.1, animation: `barShimmer 2.2s ease-in-out ${idx * 0.15}s infinite` }} />
      ))}
    </div>
  );
  if (type === 'latency') return (
    <div style={wrap}>
      <svg viewBox="0 0 220 92" width="100%" height="100%" preserveAspectRatio="none">
        <polyline style={{ strokeDasharray: '6 4', animation: 'dashFlow 1.2s linear infinite' }} points="0,70 30,68 60,40 90,58 120,22 150,34 180,16 220,20" fill="none" stroke={color} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
      </svg>
    </div>
  );
  if (type === 'auth') return (
    <div style={{ ...wrap, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <div style={{ width: 50, height: 50, borderRadius: '50%', border: `1.5px solid ${color}`, opacity: 0.3, position: 'absolute', animation: 'shieldRing 2.4s ease-out infinite' }} />
      <i className="ti ti-key" style={{ color, fontSize: 28, position: 'relative', zIndex: 1 }} />
    </div>
  );
  if (type === 'uptime') return (
    <div style={{ ...wrap, display: 'flex', alignItems: 'center', padding: '0 14px', gap: 3 }}>
...
══════════════════════════════════════════════════ */
const accentMap = {
  dark:  { bg: '#0e1411', border: 'rgba(34,197,94,0.16)', bar: 'linear-gradient(90deg,#16a34a,#22c55e,#4ade80)', iconBg: 'rgba(34,197,94,0.12)', iconColor: '#4ade80', tagColor: '#4ade80', titleColor: '#fff', descColor: 'rgba(255,255,255,0.6)', featColor: 'rgba(255,255,255,0.52)', hoverBorder: 'rgba(34,197,94,0.45)', hoverShadow: '0 24px 60px -12px rgba(22,163,74,0.35)', ctaTextHover: '#06140c' },
  mid:   { bg: '#06150c', border: 'rgba(34,197,94,0.14)', bar: 'linear-gradient(90deg,#14532d,#16a34a,#22c55e)', iconBg: 'rgba(22,163,74,0.18)', iconColor: '#86efac', tagColor: '#86efac', titleColor: '#f0fdf4', descColor: 'rgba(187,247,208,0.68)', featColor: 'rgba(187,247,208,0.56)', hoverBorder: 'rgba(34,197,94,0.4)', hoverShadow: '0 24px 60px -12px rgba(5,46,22,0.55)', ctaTextHover: '#06140c' },
  light: { bg: '#ffffff', border: 'rgba(15,23,42,0.07)', bar: 'linear-gradient(90deg,#22c55e,#4ade80,#86efac)', iconBg: '#f0fdf4', iconColor: '#15803d', tagColor: '#16a34a', titleColor: '#0f1c14', descColor: '#586b5d', featColor: '#3f5345', hoverBorder: 'rgba(22,163,74,0.32)', hoverShadow: '0 24px 60px -16px rgba(15,23,42,0.16)', ctaTextHover: '#fff' },
  soft:  { bg: '#ffffff', border: 'rgba(15,23,42,0.07)', bar: 'linear-gradient(90deg,#4ade80,#86efac,#bbf7d0)', iconBg: '#f0fdf4', iconColor: '#16a34a', tagColor: '#22c55e', titleColor: '#0a0a0a', descColor: '#5b6470', featColor: '#586474', hoverBorder: 'rgba(22,163,74,0.28)', hoverShadow: '0 24px 60px -16px rgba(15,23,42,0.14)', ctaTextHover: '#fff' },
};
...
function ServiceCard({ svc, index }) {
  ...
  return (
    <div ref={ref} onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)}
      ...
      <button
        style={{ background: hovered ? a.iconColor : 'transparent', border: `1.5px solid ${a.iconColor}`, color: hovered ? a.ctaTextHover : a.iconColor, borderRadius: 11, padding: '12px 20px', fontFamily: "'Poppins',sans-serif", fontWeight: 600, fontSize: 12.5, cursor: 'pointer', transition: 'all 0.3s ease', width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 7 }}>
  ...
  return (
    <div style={{ position: 'relative', width: '100%', display: 'flex', justifyContent: 'center' }}>
      <div style={{ position: 'relative', width: '100%', maxWidth: 280 }}>
        <div style={{ background: 'linear-gradient(165deg,#0d1611 0%,#0a120d 100%)', border: '1px solid rgba(34,197,94,0.22)', borderRadius: 16, padding: 14, boxShadow: '0 28px 56px -16px rgba(0,0,0,0.55)', position: 'relative', overflow: 'hidden', animation: 'deviceLaptop 4.4s ease-in-out infinite' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: 12 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 4, background: 'rgba(74,222,128,0.12)', border: '1px solid rgba(74,222,128,0.25)', borderRadius: 20, padding: '3px 8px' }}>
              <span style={{ width: 5, height: 5, borderRadius: '50%', background: '#4ade80', animation: 'pulse 2s ease-in-out infinite' }} />
          ...
          <div style={{ display: 'flex', alignItems: 'flex-end', gap: 4, height: 42, marginBottom: 14 }}>
            {bars.map((h, i) => <div key={i} style={{ flex: 1, height: `${h}%`, borderRadius: '3px 3px 0 0', background: i === bars.length - 1 ? 'linear-gradient(180deg,#4ade80,#16a34a)' : 'rgba(74,222,128,0.2)', animation: `barGrow 0.7s cubic-bezier(.22,1,.36,1) ${i * 0.07}s both` }} />)}
          </div>
        ...
        <div>
          ...
          <div className="hero-item" style={{ animationDelay: '0.32s', display: 'flex', gap: 12, flexWrap: 'wrap' }}>
            ...
            <a href="#contact" style={{ textDecoration: 'none' }}>
              <button className="hero-btn-outline">Send an Enquiry</button>
            </a>
          </div>
...
function ServicesGrid() {
  return (
    <div id="services" className="section-pad" style={{ maxWidth: 1320, margin: '96px auto', padding: '0 64px' }}>
...
function HowWeWork() {
  return (
    <div id="how" className="band-pad" style={{ background: '#0a0a0a', padding: '88px 64px', position: 'relative', overflow: 'hidden' }}>
...
function FAQ() {
  return (
    <div id="faq" className="band-pad" style={{ background: '#fafaf8', padding: '88px 64px' }}>
      ...
      <style>{`
        ...
        *::-webkit-scrollbar { display: none; }
        ...
        @media (max-width: 768px) {
          ...
          .hero-badge-a, .hero-badge-b { display:none !important; }
        ...
        @media (max-width: 480px) {
          ...
          .section-pad > h2, .band-pad h2 { font-size:28px !important; }
        ...
        @media (max-width: 360px) {
          ...
          .hero-btn-primary, .hero-btn-outline { font-size:12px !important; padding:11px 18px !important; }
        }

        @media (prefers-reduced-motion: reduce) {
          .hero-item, .hero-device-float, .hero-badge-a, .hero-badge-b { animation:none !important; }
        }
      `}</style>
